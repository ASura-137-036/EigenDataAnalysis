...
# ╔═╡ 905e40b0-909c-11f1-b893-dfef58902269
begin
    ...
    WGLMakie.activate!()
    using Revise
    include("../src/EigenDataAnalysis.jl")
    using .EigenDataAnalysis
end

# ╔═╡ d57c8a64-3f97-4f6e-b2cd-b755dcd43106
md"""
## Data Generation
"""

# ╔═╡ b3efb37b-1646-416e-ba9e-e8b15180254a
begin
    # Generate 2-sensor noisy outputs for a 3-DOF system
    dt = 0.01
    t = 0:dt:10
    N_pts = length(t)
    ny = 2
    ...
    f_true = [2.5, 6.0, 12.0]
    z_true = [0.02, 0.05, 0.01]
    
    
    X_clean = zeros(ny, N_pts)
    for i in 1:length(f_true)
        omega_d = 2π * f_true[i] * sqrt(1 - z_true[i]^2)
        sigma = -2π * f_true[i] * z_true[i]
        phi = randn(ny) 
        X_clean += phi * (exp.(sigma .* t) .* sin.(omega_d .* t))'
    end
    
    # 10% noise addition to test numerical stability
    X_noisy = X_clean + (0.1 * std(X_clean)) * randn(ny, N_pts)*2
    
    # WGLMakie Plot
    fig_data = Figure(size = (800, 400))
    ax_data = Axis(fig_data[1, 1], xlabel="Time [s]", ylabel="Amplitude", title="Generated 3-DOF Sensor Data")
    
    lines!(ax_data, t, X_noisy[1, :], color=(:gray, 0.6), label="Noisy Data (Sensor 1)")
    lines!(ax_data, t, X_clean[1, :], color=:black, linewidth=2, label="Clean Baseline")
    
    axislegend(ax_data, position=:rt)
    fig_data
end

# ╔═╡ dde0e120-9f68-464e-8203-38277cfc5a8a
σ_true = -2π .* f_true .* z_true

# ╔═╡ f80fb32b-09cb-45c0-89ee-06015f53a9e5
md"""
## Fit Models & Dynamic Reconstructions
"""

# ╔═╡ 24a8f8c9-e778-447c-ad6b-29190b4e1357
begin
    L_lag = 50 
    R_order = 8
    
    # Fit the models
    m_cov  = EigenDataAnalysis.fit(SSICOV, X_noisy, dt, max_lag=L_lag, r=R_order)
    m_pc   = EigenDataAnalysis.fit(PCSSI, X_noisy, dt, max_lag=L_lag, r=R_order)
    m_data = EigenDataAnalysis.fit(SSIDATA, X_noisy, dt, max_lag=L_lag, r=R_order)
    
    # Extract aligned reconstructions
    Y_cov, t_cov_idx   = EigenDataAnalysis.reconstruct(m_cov)
    Y_pc, t_pc_idx     = EigenDataAnalysis.reconstruct(m_pc)
    Y_data, t_data_idx = EigenDataAnalysis.reconstruct(m_data)
    
    # Compute and print errors
    err_cov  = EigenDataAnalysis.reconstruction_error(m_cov,  X_clean)
    err_pc   = EigenDataAnalysis.reconstruction_error(m_pc,   X_clean)
    err_data = EigenDataAnalysis.reconstruction_error(m_data, X_clean)
    println("Errors -> COV: $(round(err_cov, digits=4)), PC: $(round(err_pc, digits=4)), DATA: $(round(err_data, digits=4))")
    
    # WGLMakie Plot (Zoomed in to first 400 points to see the fit)
    fig_recon = Figure(size = (800, 450))
    ax_recon = Axis(fig_recon[1, 1], xlabel="Time [s]", ylabel="Amplitude", title="Signal Reconstruction (First 400 points)")
    
    plot_limit = 400
    
    # Base noisy data
    lines!(ax_recon, t[1:plot_limit], X_noisy[1, 1:plot_limit], color=(:gray, 0.4), label="Original Noisy")
    
    # COV overlaps perfectly from start
    lines!(ax_recon, t[t_cov_idx][1:plot_limit], Y_cov[1, 1:plot_limit], 
           color=:blue, linewidth=2, label="SSI-COV")
           
    # DATA is shifted due to past/future block matrices. Ensure we don't index past the array.
    limit_data = min(plot_limit, length(t_data_idx))
    lines!(ax_recon, t[t_data_idx][1:limit_data], Y_data[1, 1:limit_data], 
           color=:orange, linewidth=2, label="SSI-DATA")

	lines!(ax_recon, t[t_data_idx][1:limit_data], Y_pc[1, 1:limit_data], 
           color=:violet, linewidth=2, label="SSI-pc")
    
    axislegend(ax_recon, position=:rt)
    fig_recon
end

# ╔═╡ f9c1171c-e183-4dbc-aeec-597c092f4b31
md"""
## Pole Distribution (Eigenvalues)
"""

# ╔═╡ 1fdedf5b-971d-4eae-ba58-beb1d5219c25
begin
    # Extract continuous poles
    e_cov = EigenDataAnalysis.eigenvalues(m_cov)
    e_pc = EigenDataAnalysis.eigenvalues(m_pc)
    e_data = EigenDataAnalysis.eigenvalues(m_data)
    
    # WGLMakie Figure
    fig_poles = Figure(size = (800, 500))
    ax_poles = Axis(fig_poles[1, 1], 
              xlabel = "Damping Rate σ (Real)", 
              ylabel = "Damped Freq fd (Imag / 2π)", 
              title = "Continuous Poles (Observability vs. State Sequence Shift)")
    
    # Observability Shift
    scatter!(ax_poles, real.(e_cov), imag.(e_cov) ./ 2π, 
             label = "SSI-COV (O-Shift)", marker = :circle, markersize = 14, color = (:blue, 0.5))
             
    # State Sequence Shifts
    scatter!(ax_poles, real.(e_pc), imag.(e_pc) ./ 2π, 
             label = "PC-SSI (X-Shift)", marker = :rect, markersize = 10, color = (:orange, 0.8))
             
    scatter!(ax_poles, real.(e_data), imag.(e_data) ./ 2π, 
             label = "SSI-DATA (X-Shift)", marker = :cross, markersize = 10, color = (:green, 1.0))
    
    # Stability boundary
    vlines!(ax_poles, [0.0], color = :red, linestyle = :dash, linewidth = 2, label = "Stability Boundary")
    
    axislegend(ax_poles, position = :lb)
    fig_poles
end

# ╔═╡ f565c5f0-ef30-47f2-848e-40d4369e5a9f
WGLMakie.available_marker_symbols()

# ╔═╡ d81ec0c0-ff1e-4c2c-b027-d00029ff364a
let
    test_orders = 2:1:20
    
    # 1. Compute stabilization points for all three architectures
    ...
    pts_pc, _, _ = EigenDataAnalysis.compute_stabilization_diagram(
        PCSSI, X_noisy, dt; orders=test_orders, max_lag=50,
        tol_f=0.01, tol_zeta=0.05, tol_mac=0.95
    )
    
    pts_data, _, _ = EigenDataAnalysis.compute_stabilization_diagram(
        SSIDATA, X_noisy, dt; orders=test_orders, max_lag=50,
        tol_f=0.01, tol_zeta=0.05, tol_mac=0.95
    )
    ...
    function plot_multi_status_diagram!(ax, points, true_freqs)
        status_specs = [
            (:fully_stable, "Fully Stable (f, ζ, MAC)", :circle,   :green,       10),
            (:freq_damp,   "Stable (f, ζ)",             :utriangle, :orange,      8),
            (:freq_mac,    "Stable (f, MAC)",           :rect,     :purple,      8),
            (:freq,        "Stable (f only)",           :cross,    :deepskyblue, 7),
            (:unstable,    "Unstable / Vector Drift",   :xcross,   (:gray, 0.6), 6),
            (:new,         "New Pole",                  :star4,      (:violet, 0.6), 10)
        ]
        
        # Ground truth frequency reference
        vlines!(ax, true_freqs, color=(:red, 0.5), linestyle=:dash, linewidth=1.5, label="True Frequencies")
...
manifest_format = "2.0"
project_hash = "de0e0b62ffae3386cf5f39a79b70153c57f05ff1"
